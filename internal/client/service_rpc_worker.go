...
func (s *ClientRPCService) StartProfileWorker(ctx context.Context, driver ClientRPCProfileDriver) (<-chan error, error) {
	...
	s.profileWorker = w
	s.mutations.setProfileWorkerReadiness(w, true)
	stopReadiness := context.AfterFunc(ctx, func() { s.mutations.setProfileWorkerReadiness(w, false) })
	done := make(chan error, 1)
	go func() {
		var err error
		defer func() {
			stopReadiness()
			s.mutations.setProfileWorkerReadiness(w, false)
			s.profileMu.Lock()
			s.profileWorker = nil
			s.profileMu.Unlock()
			close(w.done)
