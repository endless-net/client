...
func TestFirstExitRecoveryRequiresBoundRunningJournal(t *testing.T) {
	for _, scenario := range []string{"running", "pending", "terminal", "missing_operation", "owner", "record_owner", "profile", "origin", "intent", "node", "wrong_kind"} {
		t.Run(scenario, func(t *testing.T) {
			...
			calls := 0
			guard, err := newLinuxExitGuard("endlessnet", 51820, func(context.Context, string, string, ...string) ([]byte, error) { calls++; return nil, nil })
			if err != nil {
				t.Fatal(err)
			}
