...
7. Сохранённые `pending`/`rejected` не прекращают чтение подписанных map:
   агент должен наблюдать последующее решение Coordinator без замены identity.
   До одобрения он не отправляет online heartbeat; доступ определяется
   проверенной картой, а не локальным предположением о новом решении.
   После первого snapshot агент ожидает map-stream события и revision
...
Conntrack-исключение теперь ограничено `REPLY` на туннельном интерфейсе:
исходное направление уже установленного соединения не обходит текущие ACL.
Ответы сохраняют исключение, поскольку их destination port — временный порт
инициатора. Это не принудительный сброс соединений и не доказательство отзыва
...
1. Подтвердить оставшиеся направления ACL, SNAT и exit policy на Windows/macOS.
   ...
   ОС; прямой IPv4/IPv6 TCP/UDP и ICMP echo внутри туннеля с IPv4-транспортом,
   выборочный отзыв разрешений и восстановление подтверждены
   [восьмиплатформенным CI](../headless-test-coverage.md#linux-arm-coverage-expansion),
   включая Linux arm64.
