...
failure, live revocation, natural expiry of an unchanged policy and renewed
...
Default-off, short-grant reporting, immutable retry, revocation, natural expiry
and renewal passed 23 native Linux/Windows/macOS repetitions at source `1af0278`.
One macOS Intel repetition did not compile after a dependency DNS failure, so
complete matrix qualification remains open; see the
[coverage ledger](headless-test-coverage.md#2026-09-12-23-native-reports-passed-one-build-missing).
