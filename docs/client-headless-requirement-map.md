...
| IT-30 | Exit apply/clear, locked settings and resource conflicts | I `service_rpc_preferences_test.go` covers validation only; `service_rpc_exit_catalog_test.go` covers signed live-grant catalog; `exit_routes_test.go` and `exit_filter_test.go` cover isolated route and packet policy | Route/filter layers are not wired to the engine or OS fail-closed rules; apply/clear/status, resources runtime and remaining settings missing; signed grants do not prove local effects |
