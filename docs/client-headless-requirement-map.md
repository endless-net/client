...
| IT-31 | UI quit versus crash; OS lifecycle and stream recovery | I `service_rpc_uiquit_test.go`; `service_rpc_events_test.go`; Windows SCM power delivery and agent gate/retry/shutdown: `windows_service_lifecycle_windows_test.go`, `agent_runtime_lifecycle_test.go`; Linux logind inhibitor and UID-bound session delivery: `agent_lifecycle_linux_test.go` | Windows power/logoff and Linux logind power/logoff feed the runtime; macOS source, native sleep/logoff/SCM timing, source-health projection and complete recovery audit remain open; do not add untrusted OS events to UI notification RPC |
