...
| IT-30 | Exit apply/clear, locked settings and resource conflicts | I `service_rpc_preferences_test.go` covers validation only; `service_rpc_exit_catalog_test.go` covers signed live-grant catalog | Exit apply/clear/status, resources runtime and remaining policy settings missing; pinned clientapi supplies signed exit grants but does not prove local effects |
