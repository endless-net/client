...
| IT-31 | UI quit versus crash; OS lifecycle and stream recovery | I `service_rpc_uiquit_test.go`; `service_rpc_events_test.go`; Windows SCM power delivery and agent gate/retry/shutdown: `windows_service_lifecycle_windows_test.go`, `agent_runtime_lifecycle_test.go` | Windows power/logoff paths and signed resume-policy refresh wired; logoff source-health recovery, non-Windows sources, observation and native qualification remain open; do not add untrusted OS events to UI notification RPC |
