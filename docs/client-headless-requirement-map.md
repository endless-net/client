...
| IT-30 | Exit apply/clear, locked settings and resource conflicts | I `service_rpc_preferences_test.go` covers validation only; `service_rpc_exit_catalog_test.go` covers signed live-grant catalog; `exit_routes_test.go` covers isolated selection-bound route derivation | Exit route derivation is not wired to engine/enforcement; apply/clear/status, resources runtime and remaining policy settings missing; signed grants do not prove local effects |
