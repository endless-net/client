...
| IT-31 | UI quit versus crash; OS lifecycle and stream recovery | I `service_rpc_uiquit_test.go`; `service_rpc_events_test.go`; Windows SCM power delivery and agent gate/retry/shutdown: `windows_service_lifecycle_windows_test.go`, `agent_runtime_lifecycle_test.go`; Linux logind inhibitor completion/deadline: `agent_lifecycle_linux_test.go` | Windows power/logoff and Linux logind sleep paths feed signed resume-policy refresh; Linux logoff, macOS source, native sleep/SCM timing, source-health projection and complete recovery audit remain open; do not add untrusted OS events to UI notification RPC |
