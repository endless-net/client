...
| IT-15 | ACL withdrawal and local inbound restriction | I `application_runtime_test.go`; `map_expiry_filter_test.go`; `service_rpc_inbound_preferences_test.go`; `inbound_tun_test.go` | Inbound preference, atomic patch/reset and managed-lock rejection implemented; full enforcement/revoke-bound assertion audit and real control-flow evidence remain |
| IT-16 | DNS/routes projection and local acceptance preferences | I `application_runtime_test.go`; `service_rpc_network_preferences_public_test.go`; `service_rpc_network_preferences_worker_test.go`; `network_preferences_test.go` | Public admission/worker, pending versus committed settings and policy/reset implemented; actual resolver/routes effects and full recovery audit remain; route approval belongs to backend |
...
| IT-28 | Profile context isolation and removal guards | I `service_rpc_profiles_test.go`; `service_rpc_switch_test.go`; network activation/apply/public assertions inspected in [current execution review](client-cutover-execution.md#corrected-network-selection-evidence) | Cross-network worker and native driver are wired; full context/provider race audit and actual route isolation remain open |
...
| IT-30 | Exit apply/clear, locked settings and resource conflicts | I `service_rpc_exit_public_test.go`, `service_rpc_exit_status_test.go`, `service_rpc_exit_executor_test.go`, `service_rpc_exit_worker_test.go`; `exit_guarded_engine_test.go`, `exit_route_applied_test.go`, `exit_policy_applied_test.go`, `exit_family_absent_test.go`; no implicit defaults: `exit_engine_test.go`, `exit_export_test.go` | Public Get/Select/Clear, durable retry/containment and isolated Linux engine observations implemented; production native executor/worker startup, complete Clear/release crash recovery, firewall/DNS/ownership and OS/LAN qualification remain. Resources/settings have their own partial implementations; injected tests do not prove OS effects |
| IT-31 | UI quit versus crash; OS lifecycle and stream recovery | I `service_rpc_uiquit_test.go`; `service_rpc_events_test.go`; Windows SCM power delivery and agent gate/retry/shutdown: `windows_service_lifecycle_windows_test.go`, `agent_runtime_lifecycle_test.go`; Linux logind inhibitor and UID-bound session delivery: `agent_lifecycle_linux_test.go`; macOS IOKit completion/deadline: `agent_lifecycle_darwin_test.go` | Windows power/logoff, Linux logind power/logoff and macOS IOKit power feed the runtime; macOS UID-bound logoff, native sleep/logoff/SCM timing, source-health projection and complete recovery audit remain open; do not add untrusted OS events to UI notification RPC |
| IT-32 | Private diagnostics access, bounded export and resource context | I `service_rpc_bundle_read_test.go`; `service_rpc_bundle_admin_test.go`; `service_rpc_resources_test.go` | Signed catalog search and durable resource enablement exist; observations confirm committed denial filtering, not positive path availability. Complete route/resource diagnostics and independent audit of every bundle bound and authorization case remain open |
...
| BR-02 | AC-02 | Human enrollment and independent session recovery; IT-01–03, IT-20/29 | Session worker is started by Serve when configured or recovering a journal; full cleanup/concurrency audit and producer-backed IdP/approval/renewal evidence remain |
...
| BR-06 | AC-06 | Durable intent and lifecycle settings; IT-22/26/31 | Lifecycle fields and policy resolution implemented; complete native delivery/effects and recovery audit remain |
...
| BR-09 | AC-09 | DNS projection and external resolver preference; IT-16 | Accept-DNS admission/policy/worker implemented; full effect/recovery audit and real resolver behavior remain |
| BR-10 | AC-10 | Revocation and local restriction intersection; IT-08–10/15/30 | Local inbound preference and managed-policy intersection implemented; Q-04 revoke bounds, full assertion audit and platform evidence remain |
...
| BR-12 | AC-12 | Subnet/resource routing and conflicts; IT-16/30 | Resource API/preferences and committed denial filters implemented; positive path observations, complete overlap/recovery audit and Q-07 remain; hosting/provider responsibilities stay distinct |
| BR-13 | AC-13 | Authorized exit selection, LAN and fail-closed behavior; IT-30 | Linux native adapter/host/worker, durable release recovery and readback are implemented. LAN semantics are accepted in D-034 (`8719ce7`); candidate source and exact reservation subtraction do not yet implement LAN datapath. Full OS/LAN effects and qualification remain. Other Q-07 role variants stay external |
