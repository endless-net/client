...
| IT-30 | Exit apply/clear, locked settings and resource conflicts | I `service_rpc_preferences_test.go` covers validation only; `service_rpc_exit_catalog_test.go` covers signed live-grant catalog; `exit_routes_test.go`, `exit_filter_test.go`, `exit_tun_test.go` cover route policy, packet policy and injected TUN composition | Live engine activation and OS fail-closed rules are not wired; apply/clear/status, resources runtime and remaining settings missing; injected TUN tests do not prove OS effects |
