...
| IT-31 | UI quit versus crash; OS lifecycle and stream recovery | I `service_rpc_uiquit_test.go`; `service_rpc_events_test.go`; Windows SCM power delivery and agent gate/retry/shutdown: `windows_service_lifecycle_windows_test.go`, `agent_runtime_lifecycle_test.go` | Windows power path wired; logoff, non-Windows sources, suspended policy refresh, observation and native qualification remain open; do not add untrusted OS events to UI notification RPC |
