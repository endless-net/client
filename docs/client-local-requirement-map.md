...
| US-10 | `service_rpc_preferences.go`, `service_rpc_uiquit.go`, `network_preferences.go`, `service_rpc_network_preferences_worker.go` | Preference/UI-quit validation plus `TestNetworkPreferenceWorkerApplyAndContainment` and `TestNetworkPreferenceContainmentRecoversAfterDownFailure`: candidate delivery, atomic commit, preserved Disconnect intent, distinct stale/source/apply failures and durable Down recovery | Public DNS/routes admission/projection and effect observations remain open; inbound and other lifecycle keys, full policy/atomic-patch coverage and OS effects are incomplete |
...
| US-12 | `service_rpc_uiquit.go`, host lifecycle | `service_rpc_uiquit_test.go`, `service_rpc_events_test.go` | Real OS startup/logoff/suspend/resume and managed lifecycle effects |
