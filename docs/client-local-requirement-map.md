...
| US-10 | `service_rpc_preferences.go`, `service_rpc_uiquit.go`, `network_preferences.go`, `service_rpc_network_preferences_worker.go`, `service_rpc_network_preferences_public.go` | Worker apply/containment tests plus `TestNetworkPreferencePublicAdmissionWakesWorkerAndSeparatesPending` and `TestNetworkPreferenceProjectionManagedLockAndReset`: pending/committed projection, wakeup, signed locks and reset | Authenticated transport acceptance and OS effect observations remain open; inbound and other lifecycle keys and full policy/atomic-patch coverage are incomplete |
