...
| US-04 | Network catalog/selection, registration/cleanup providers, durable activation/apply worker and native profile driver; peer projection | `service_rpc_network_public_test.go`, activation/apply/abort recovery assertions described in [current execution review](client-cutover-execution.md#corrected-network-selection-evidence); peer tests | Selection is wired; complete cross-worker/context assertion audit and actual identity/map/routes isolation and recovery acceptance remain open |
...
2026-09-23 US-11 flow evidence: `resource_flow_observation_test.go` asserts
correlated TCP/UDP responses, denied/malformed/failed-delivery branches, exact
signed service/subnet mapping, both advertised families and overlapping peer
ownership. `TestResourceFlowCollectorBindsSignedServiceSubnetAndLiveRoute`
exercises the configured in-memory engine, signed map, injected native readback
including a foreign physical route, ListResources/event projection and receipt
invalidation. The production TUN wrapper, route/path collector and ListResources
clock now use these short-lived samples; stale map, policy, profile, topology,
exit and reapply still invalidate through the existing receipt binding. A
sample proves one delivered reply, not subnet-wide or semantic application
health. No native/system run has qualified these effects; the full US-11 row
and cross-worker transition audit remain open.
| US-12 | `service_rpc_uiquit.go`, `service_rpc_runtime_start.go`, signed lifecycle resolver, agent startup before workers; per-profile desktop preferences; `windows_service_windows.go` power queue, `agent_lifecycle_linux.go` logind delay/session source and `agent_lifecycle_darwin.go` IOKit power source → `agent_runtime_lifecycle.go` → shared engine gate | UI-quit/events; runtime-start source/lock/reset/replay; no implicit startup connection; desktop policy/default/reset/restart and shared atomic worker; `windows_service_lifecycle_windows_test.go` delivery/overflow, `agent_runtime_lifecycle_test.go` gate/retry/wake/source loss/shutdown; Linux source inhibitor/UID session/reconnect/overflow units; macOS power completion/deadline units; runtime-policy refresh CAS/authority and source/executor recovery units; lifecycle observation ordering, failure, snapshot/privacy/no-probe units; `runtime_lifecycle_events_test.go` owner/observer streams, reattach and reopened-runtime evidence; `windows_session_owners_test.go` owner binding, stale lookup, enumeration/logoff races and bounds; dispatcher units verify callback copying, overflow and terminal status ordering | macOS UID-bound logoff source, full native/subscriber observation audit, complete recovery-race audit and native lifecycle qualification; logind deadline limits sleep coordination |
