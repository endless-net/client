...
| US-12 | `service_rpc_uiquit.go`, `service_rpc_runtime_start.go`, signed lifecycle resolver, agent startup before workers; per-profile logoff/suspend/resume preferences with accepted KEEP_INTENT defaults | UI-quit/events tests; `service_rpc_runtime_start_test.go` covers source/lock/reset/replay, startup intent and pending recovery; `agent_startup_intent_test.go` proves no control-plane request without saved intent; `service_rpc_desktop_preferences_test.go` covers policy/default/reset/restart and atomic rejection; shared worker tests include all desktop settings | Real logoff/suspend/resume inputs and CONNECT execution, complete recovery-race audit and platform lifecycle qualification |
