...
| US-02 enrollment | `service_rpc_enrollment_worker.go`, `service_rpc_enrollment_executor.go`, credential trust included in `copyEnrollmentFields` | `TestRPCEnrollmentWorkerRecoveryAndShutdown`, `TestEnrollmentCheckpointPersistsCredentialVerificationAuthority`, `TestEnrollmentCheckpointRejectsConcurrentCredentialTrustChange` | Credential trust now survives RPC checkpoint/disk reopen and participates in checkpoint CAS. Direct native-enrollment tests previously bypassed this callback. Trace remaining approval, denial, expiry, cancellation and ownership outcomes individually; actual backend approval is external. |
...
| US-04 networks/peers | `service_rpc_networks.go`, `service_rpc_select_network.go`, `service_rpc_peers.go` | `TestRPCSelectCurrentNetworkIsDurableNoop`; peer pagination/event suites | Source audit confirms different-network selection returns UNSUPPORTED and catalog entries advertise no running selector. UF-07/UBR-10/UI-AC-14 still require authorized target registration, stopped old routes, isolated target map/identity and durable recovery; current-network no-op does not meet this. |
