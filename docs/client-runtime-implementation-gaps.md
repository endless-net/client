...
| US-04 networks/peers | `service_rpc_networks.go`, `service_rpc_select_network.go`, `service_rpc_peers.go`; exact account-bound registration; isolated target preparation; internal durable `service_rpc_network_selection_plan.go` | Current-network no-op and exact-ID units, target isolation/catalog rejection tests, `TestNetworkSelectionPreparationPersistsIsolatedPlanAndReplay`, `TestNetworkSelectionPreparationRejectsConcurrentSourceChange`; peer suites | Public different-network selection still returns UNSUPPORTED. Internal preparation now saves a private source/target plan with operation replay and CAS after catalog retrieval; disk reopen reuses the checkpoint without activating it. Concurrent intent/owner/profile changes reject the result. UF-07/UBR-10/UI-AC-14 still require registration checkpoints, revalidation at handover, old-route teardown, activation, cancellation/terminal cleanup and recovery. Preparation is not full selection acceptance. |
