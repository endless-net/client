...
| US-04 networks/peers | `service_rpc_networks.go`, `service_rpc_select_network.go`, `service_rpc_peers.go`; native registration now accepts an exact runtime NetworkID and signs selected account/session binding | `TestRPCSelectCurrentNetworkIsDurableNoop`, `TestEnrollmentExactNetworkIDAndAccountBinding`, `TestExactNetworkEnrollmentRejectsMixedContextBeforeCheckpoint`; peer pagination/event suites | Different-network selection still returns UNSUPPORTED and catalog entries advertise no running selector. The exact-ID registration primitive rejects mixed old node/credential/map or name/token authorization, but is not yet wired into a selector. UF-07/UBR-10/UI-AC-14 still require fresh catalog authorization, durable target preparation, stopped old routes, target activation and recovery; current-network no-op does not meet this. |
