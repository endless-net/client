...
| US-04 networks/peers | `service_rpc_networks.go`, `service_rpc_select_network.go`, `service_rpc_peers.go`; exact account-bound native registration; `PrepareNetworkSelectionTarget` reads fresh account catalog and builds isolated target configuration | `TestRPCSelectCurrentNetworkIsDurableNoop`, exact-ID enrollment tests, `TestNetworkSelectionTargetSeparatesOldNetworkState`, `TestNetworkSelectionTargetRejectsUnauthorizedOrInvalidCatalog`; peer pagination/event suites | Different-network selection still returns UNSUPPORTED. Target preparation rejects mixed catalog accounts/origins, absent target, duplicates, oversized data and cancellation; it preserves installation/account authority without old node/map/exit/resource/recovery or config aliases. It does not persist or activate the target. UF-07/UBR-10/UI-AC-14 still require coordinator context CAS, durable preparation, old-route teardown, activation and recovery; neither preparation nor current-network no-op meets full selection acceptance. |
