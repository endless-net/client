...
| US-04 networks/peers | Network/peer handlers; exact account-bound registration; isolated target preparation; durable plan/checkpoint; native `agentRPCRegisterNetworkTarget` registers or refreshes an existing target through a full signed map | Current-network, exact-ID, preparation/replay/CAS and registration checkpoint units; `TestNetworkTargetApprovalRefreshDoesNotReregister` covers approval, tampered signature and signed foreign-account response; peer suites | Public different-network selection still returns UNSUPPORTED. Target provider now uses native registration and polls an existing node without re-registering; its full coordinator/worker path is not wired. Private checkpoints bind operation, account/network, session and keys, preserving active source. Registration outcome reconciliation, handover revalidation, old-route teardown, activation, cancellation/terminal cleanup and recovery remain required for UF-07/UBR-10/UI-AC-14. Provider units are not full selection acceptance. |
