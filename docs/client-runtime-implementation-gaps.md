...
| US-04 networks/peers | Network/peer handlers; exact account-bound registration; isolated target preparation; durable selection plan and `NetworkSelectionSaveCallback` | Current-network, exact-ID, preparation/replay/CAS units; `TestNetworkRegistrationCheckpointStaysInTargetAcrossRestart`, `TestNetworkRegistrationCheckpointRejectsChangedBindings`, `TestNetworkRegistrationCheckpointBindsSignedRequest`; peer suites | Public different-network selection still returns UNSUPPORTED. Private target checkpoints preserve active source and bind operation, account/network, session and installation keys; a signed registration request cannot change on retry. Provider signature verification, registration execution, handover revalidation, old-route teardown, activation, cancellation/terminal cleanup and recovery must still be connected and audited for UF-07/UBR-10/UI-AC-14. Checkpoint storage is not selection acceptance. |
