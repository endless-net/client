...
| Preferences и lifecycle | Set/Reset inbound/DNS/routes и lifecycle-полей, managed policy/locks/source; Windows power/logoff paths с ожиданием suspend teardown в SCM callback (25 с) и resume-policy refresh; Linux logind sleep + UID-bound session removal с source recovery; отсутствующие OS-event sources дают UNSUPPORTED | Полный аудит эффектов каждой настройки, macOS daemon sources и Darwin core artifact отсутствуют (AppKit session events не доказывают UID-bound logout; IOKit sleep требует native callback/ack), native sleep/logoff/SCM qualification; задержка logind ограничена `InhibitDelayMaxUSec` |
