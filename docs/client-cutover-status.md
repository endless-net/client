...
| Preferences и lifecycle | Set/Reset inbound/DNS/routes и lifecycle-полей, managed policy/locks/source; Windows power/logoff paths и resume-policy refresh; Linux logind sleep source с delay FD и подтверждением teardown; отсутствующие OS-event sources дают UNSUPPORTED | Полный аудит эффектов каждой настройки, Linux logoff и macOS sources, native sleep/SCM qualification; задержка logind ограничена `InhibitDelayMaxUSec` |
